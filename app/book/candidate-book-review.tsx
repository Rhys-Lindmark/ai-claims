      ...
      <section className="mt-14">
        ...
        <div className="mt-5 divide-y divide-[#20211f]/10 border-y border-[#20211f]/10">
          {packet.claims.map((claim, index) => <article className="py-6" key={claim.claim_id}>
            ...
            {claim.evidence_record_ids.length > 0 ? <details className="mt-3 text-sm"><summary className="cursor-pointer font-semibold text-[#20211f]/55">{claim.evidence_record_ids.length} evidence {claim.evidence_record_ids.length === 1 ? 'note' : 'notes'}</summary><div className="mt-3 space-y-4">{claim.evidence_record_ids.map((evidenceId) => {
              const evidence = packet.evidence_records.find((item) => item.evidence_id === evidenceId)!;
              const source = packet.sources.find((item) => item.source_id === evidence.source_id)!;
              return <div className="border-l border-[#20211f]/20 pl-4" key={evidenceId}><p className="text-xs font-semibold capitalize text-[#20211f]/40">{evidence.direction}</p><p className="mt-1 leading-relaxed text-[#20211f]/70">{evidence.finding}</p><p className="mt-2 text-xs leading-relaxed text-[#20211f]/45">{evidence.scope_and_limits}</p><a className="mt-2 inline-flex items-start gap-1 text-xs font-semibold underline" href={source.url} target="_blank" rel="noreferrer">{source.title} <ExternalLink className="mt-0.5 h-3 w-3 shrink-0" /></a>{evidence.review_state ? <p className="mt-2 text-[11px] text-[#20211f]/35">Machine draft · {evidence.evidence_basis} · {evidence.confidence} confidence · checked {evidence.last_verified_at}</p> : null}</div>;
            })}</div></details> : null}
          </article>)}
        </div>
      </section>
