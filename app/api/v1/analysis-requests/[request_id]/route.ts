...
import { ANALYSIS_REQUEST_CONTRACT_VERSION, getAnalysisRequestById } from '@/lib/analysis-request-store';
...
export async function GET(_request: Request, context: { params: Promise<{ request_id: string }> }) {
  ...
  const record = await getAnalysisRequestById(getD1(), requestId);
  if (!record) return Response.json({ contract_version: ANALYSIS_REQUEST_CONTRACT_VERSION, request_id: requestId, analysis_request: null }, { status: 404, headers });
  return Response.json({ contract_version: ANALYSIS_REQUEST_CONTRACT_VERSION, request_id: requestId, analysis_request: record }, { status: 200, headers });
}
