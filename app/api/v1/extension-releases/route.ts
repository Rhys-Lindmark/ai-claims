...
export async function GET(request: Request) {
  ...
  const availableVersions = [release, release020, release019, release018, release017, release016].map((entry) => ({ version: entry.extension_version, immutable_url: `${versionBaseUrl}/${entry.extension_version}`, package_digest: entry.package.integrity.digest_hex }));
