import currentRelease from '@/releases/extension-v0.2.19.json';
...
export async function GET(request: Request, context: { params: Promise<{ version: string }> }) {
  ...
  const release = [currentRelease, release018, release017, release016].find((entry) => entry.extension_version === version) ?? currentRelease;
