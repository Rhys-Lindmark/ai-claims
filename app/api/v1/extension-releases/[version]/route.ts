import currentRelease from '@/releases/extension-v0.2.20.json';
...
export async function GET(request: Request, context: { params: Promise<{ version: string }> }) {
  ...
  const release = [currentRelease, release019, release018, release017, release016].find((entry) => entry.extension_version === version) ?? currentRelease;
