...
const starterBooks: Array<BookIdentity & { aliases: string[] }> = [
  {
...
export function BookChecker() {
  ...
  useEffect(() => {
    const initial = new URLSearchParams(window.location.search).get('book');
    if (initial) setQuery(initial);
  }, []);

  async function checkBook(event: FormEvent) {
    ...
    try {
      ...
      if (resolver.ok) {
        const envelope = await resolver.json();
      ...
      if (requestLookup.ok) {
        const envelope = await requestLookup.json();
        const request = envelope.analysis_request;
        setResult({ identity, state: request.state, requestId: request.request_id });
        return;
      }
      ...
      const envelope = await submission.json();
      setResult({ identity, state: envelope.analysis_request.state, requestId: envelope.analysis_request.request_id });
    } catch {
  ...
  return <main className="min-h-screen bg-[#fbfbfa] text-[#20211f]">
    <header className="border-b border-[#20211f]/10">
      <div className="mx-auto flex max-w-3xl items-center justify-between px-5 py-5">
        <a className="text-sm font-black tracking-tight" href="/">AI Claims</a>
    ...
    <div className="mx-auto max-w-3xl px-5 py-16 sm:py-24">
      <section className="mx-auto max-w-2xl text-center">
        ...
        <form className="mt-9" onSubmit={checkBook}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <input autoComplete="off" autoFocus className="min-w-0 flex-1 rounded-xl border border-[#20211f]/20 bg-white px-5 py-4 text-base outline-none transition placeholder:text-[#20211f]/35 focus:border-[#20211f] focus:ring-4 focus:ring-[#20211f]/5" id="book-query" onChange={(event) => setQuery(event.target.value)} placeholder="Guns, Germs, and Steel" value={query} />
      ...
      <section className="mx-auto mt-16 max-w-2xl border-t border-[#20211f]/10 pt-8">
        <h2 className="text-sm font-black uppercase tracking-[.12em] text-[#20211f]/40">Starting with</h2>
        <div className="mt-4 divide-y divide-[#20211f]/10 border-y border-[#20211f]/10">
          {starterBooks.map((book) => <button className="flex w-full items-center justify-between gap-4 py-4 text-left" key={book.entityKey} onClick={() => setQuery(book.title)}><span><strong className="block">{book.title}</strong><span className="mt-1 block text-sm text-[#20211f]/45">{book.author}</span></span><ArrowRight className="h-4 w-4 text-[#20211f]/35" /></button>)}
        </div>
        <p className="mt-5 text-sm leading-relaxed text-[#20211f]/45">Next: Rhys’s read shelf. Goodreads currently requires sign-in to access the full list, so the catalog will expand from an account export rather than scraped page data.</p>
      </section>
