...
assert.equal(packet.evidence_records.length, 15);
...
assert.deepEqual(axisClaims.map((claim) => claim.evidence_record_ids.length), [10, 1, 1]);
...
assert.equal(packet.evidence_records.filter((record) => record.claim_id === 'ggs-claim-03' && record.direction === 'complicates').length, 8);
console.log('real book packet: 8 passage-unconfirmed candidates, 15 scoped evidence records, aggregate score locked');
