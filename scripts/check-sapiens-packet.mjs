...
assert.ok(packet.claims.every((claim) => claim.evidence_record_ids.length === 0));
...
assert.equal(packet.evidence_records.length, 0);
assert.ok(!JSON.stringify(packet).match(/"score_0_100":\s*\d/));

console.log('Sapiens packet: 6 passage-unconfirmed candidates, 9 source leads, aggregate score locked');
