...
function renderResult(state, identity, requestRecord = null) {
  ...
  const releaseStatusMarkup = releaseStatus ? `<p class="meta proof"><strong>Extension ${escapeHtml(releaseStatus.state.replaceAll('_', ' '))}</strong><br>Installed ${escapeHtml(releaseStatus.installedVersion)} · published ${escapeHtml(releaseStatus.currentVersion)}<br>${escapeHtml(releaseStatus.channelLabel)}<br>${escapeHtml(releaseStatus.signingLabel)}<br>${escapeHtml(releaseStatus.digestLabel)} · ${escapeHtml(releaseStatus.privacyLabel)}</p>${releaseStatus.state === 'update_available' ? `<a class="request" href="${escapeHtml(releaseStatus.packageUrl)}" target="_blank" rel="noreferrer">Download verified update ↗</a>` : ''}` : '';
  ...
  result.innerHTML = `
    ...
    ${correctionPreview ? `<p class="meta"><strong>${escapeHtml(correctionPreview.lineage)}</strong><br>${escapeHtml(correctionPreview.summary)}</p>` : ''}
    ${compatibilityNotice ? `<p class="meta">${compatibilityNotice}</p>` : ''}
    ${state.state === 'paused' && state.analysisUrl ? `<a class="request" href="${state.analysisUrl}" target="_blank" rel="noreferrer">Open pause context ↗</a>${correctionLink ? `<a class="request" href="${escapeHtml(correctionLink.url)}" target="_blank" rel="noreferrer">${escapeHtml(correctionLink.label)} ↗</a>` : ''}` : requestRecord ? `<a class="request" href="https://ai.rhyslindmark.com/claims/request?request_id=${encodeURIComponent(requestRecord.request_id)}" target="_blank" rel="noreferrer">Open public request status ↗</a>${identity.kind === 'youtube' ? `<a class="request" href="https://ai.rhyslindmark.com/claims/intake?url=${encodeURIComponent(identity.canonicalUrl)}" target="_blank" rel="noreferrer">Supply permitted transcript ↗</a>` : identity.kind === 'goodreads' ? `<a class="request" href="https://ai.rhyslindmark.com/claims/book-intake?url=${encodeURIComponent(identity.canonicalUrl)}" target="_blank" rel="noreferrer">Confirm book edition ↗</a>` : ''}` : '<button class="request" id="request-analysis">Request analysis</button>'}
    ${deploymentProofMarkup}
    ${releaseStatusMarkup}`;
}
