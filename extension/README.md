...
For an unknown page, the extension submits only `contract_version`, `entity_key`, `canonical_url`, and coarse `page_kind` to the shared D1-backed request API. Repeat visits and other clients resolve the same canonical request. A service outage may use the existing device-only queue, labeled as a fallback; request creation never creates a score or bypasses review.
